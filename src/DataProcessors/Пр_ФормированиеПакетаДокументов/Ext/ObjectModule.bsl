...
//	* ДатаОкончания - Дата
//
Процедура СформироватьДокументы(МассивДокументов, СтруктураПараметров) Экспорт
	...
	ИспользоватьПериодФормирования = ?(СтруктураПараметров.Свойство("ДатаНачала")
									  И СтруктураПараметров.Свойство("ДатаОкончания")
									  И Месяц(СтруктураПараметров.ДатаНачала) - Месяц(СтруктураПараметров.ДатаОкончания) = 0,
									  Истина, Ложь);
	Дата = ?(ИспользоватьПериодФормирования, КонецМесяца(СтруктураПараметров.ДатаОкончания), КонецМесяца(ТекущаяДатаСеанса()));
	...
	Запрос.Текст = 
		...
		|	ЗаданиеНаРаботуРаботы.Ссылка.Пр_ФактВGitLab КАК Трудоемкость,
	...
	Пока ВыборкаКонтрагент.Следующий() Цикл
		...
		ВыборкаПроект = ВыборкаКонтрагент.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		...
		Попытка    
			...
			Пока ВыборкаПроект.Следующий() Цикл
				...
				ЗаказПокупателя.Автор = ТекущийПользователь;
				ЗаказПокупателя.БанковскийСчет = ОсновнойБанковскийСчет;
				ЗаказПокупателя.ВалютаДокумента = ОсновнаяВалюта;
				ЗаказПокупателя.ВариантЗавершения = Перечисления.ВариантыЗавершенияЗаказа.Успешно;
				ЗаказПокупателя.ВидЗаказа = Справочники.ВидыЗаказовПокупателей.Основной;
				ЗаказПокупателя.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу;  
				ЗаказПокупателя.ДатаОтгрузки = Дата; 
				ЗаказПокупателя.Договор = ОсновнойДоговор;
				ЗаказПокупателя.Касса = ОсновнаяКасса;
				ЗаказПокупателя.Комментарий = НСтр("ru = 'Автоматическое формирование документов'");
				ЗаказПокупателя.Контрагент = ВыборкаКонтрагент.Контрагент;
				ЗаказПокупателя.Кратность = ЗначенияВалюты.Кратность;
				ЗаказПокупателя.Курс = ЗначенияВалюты.Курс;
				ЗаказПокупателя.НалогообложениеНДС = ОсновноеНалогообложение;
				ЗаказПокупателя.Организация = ОсновнаяОрганизация;
				ЗаказПокупателя.Ответственный = ТекущийСотрудник;
				ЗаказПокупателя.СостояниеЗаказа = Справочники.СостоянияЗаказовПокупателей.Завершен;
				ЗаказПокупателя.СтруктурнаяЕдиницаПродажи = ПодразделениеПрограммисты;
				ЗаказПокупателя.Проект = ВыборкаПроект.Проект;
				...
				Пока Выборка.Следующий() Цикл 
					...
					СтрокаЗапасов.Сумма = Выборка.Сумма;
					...
					НоваяРабота = ЗаказПокупателя.Работы.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяРабота, СтрокаЗапасов);
										
				КонецЦикла;
				
				ЗаказПокупателя.Записать(РежимЗаписиДокумента.Проведение);				
                СоответствиеЗаказов.Вставить(ВыборкаПроект.Проект, ЗаказПокупателя.Ссылка);
			...
			Для Каждого СтрокаРабот Из АктВыполненныхРабот.РаботыИУслуги Цикл
				ЗаказПоПроекту = СоответствиеЗаказов.Получить(СтрокаРабот.Проект);
				Если ЗаказПоПроекту <> Неопределено Тогда
					СтрокаРабот.ЗаказПокупателя = ЗаказПоПроекту;	
				КонецЕсли;
			КонецЦикла;
